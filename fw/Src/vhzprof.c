...
// Take absolute frequency to allow the operation of both rotational directions
	...
	if (AbsFreq <= v->LowFreq)   
	...
	else if ((AbsFreq > v->LowFreq)&(AbsFreq <= v->HighFreq))      
		{
        	...
        	v->VoltOut = v->VoltMax;
	//Проверка нуливого задания - моя добавка, поскольку
	// при SpeedRef = 0, VdTesting = VoltMin
	// необходимо исключить наарпяжение при полной остановке привода
//	if(AbsFreq == 0)// Если 0, то обнуляем задание на напряжение
//		v->VoltOut = 0;
}
