...
typedef struct {  _iq  Valpha;   	// Input: Stationary alfa-axis stator voltage 
                  ...
                  _iq  Theta;     	// Output: Compensated rotor angle
									_iq  Mag;     	// Output: Compensated rotor angle magnitude
...
#define SMO_MACRO(v)																					\
    ...
    v.Ebeta  = v.Ebeta  + _IQmpy(v.Kslf,(v.Zbeta -v.Ebeta));											\
																										\
	/*	Rotor angle calculator -> Theta = atan2(-Ealpha,Ebeta)	*/										\
	CORDICatan2sqrt(&v.Theta,&v.Mag,_IQtoIQ13(-v.Ealpha),_IQtoIQ13(v.Ebeta)); 						
